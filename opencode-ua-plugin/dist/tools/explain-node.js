...
export const uaExplainNode = Object.assign(tool({
    description: "Explain a specific node (file, function, class, etc.) with its incoming and outgoing edges.",
    args: {
        node_id: zod.string()
    ...
    async execute(args, ctx) {
        ...
        if (!graph)
            return { incoming: [], outgoing: [], error: "no_graph" };
        const node = graph.nodes.find((n) => n.id === args.node_id);
        if (!node)
            return { incoming: [], outgoing: [], error: "not_found" };
        return {
            node: {
                id: node.id,
                name: node.name,
