...
import { GraphBuilder, type KnowledgeGraph, type GraphNode } from "@understand-anything/core";
...
type NonCodeType = "document" | "config" | "table" | "module";

function classifyNonCodeNodeType(ext: string): NonCodeType {
  if (DOC_EXTS.has(ext)) return "document";
  if (CONFIG_EXTS.has(ext)) return "config";
  if (DATA_EXTS.has(ext)) return "table";
  if (SCRIPT_EXTS.has(ext)) return "module";
  return "module";
}
...
function computeGitHash(projectRoot: string): string {
  try {
...
export async function scanProject(projectRoot: string): Promise<KnowledgeGraph> {
